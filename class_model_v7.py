...
class DeepTPC_ICLR:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            pi_sample       = tf.reduce_sum(mask_e * tf.reshape(self.pis, [-1, self.K]), axis=1)
            ...
            self.LOSS_2     = tf.reduce_mean(tf.reduce_sum(self.rnn_mask2 * loss_1(self.y, self.y_hats, self.y_type), axis=1))

            ## LOSS3: sample-wise entropy
            self.LOSS_3     = tf.reduce_mean(-tf.reduce_sum(self.rnn_mask2 * tf.reduce_sum(self.pis * log(self.pis), axis=2), axis=1))
            ...
            self.solver_E             = tf.train.AdamOptimizer(self.lr_rate1).minimize(
                self.LOSS_1 + self.delta*self.LOSS_Ey, 
    ...
    def train_mle(self, x_, y_, lr_train, k_prob):
        return self.sess.run([self.solver_MLE, self.LOSS_2],
                             feed_dict={self.x: x_, self.y: y_,
                                        self.mb_size:np.shape(x_)[0], self.lr_rate1: lr_train, self.keep_prob: k_prob})
    
    def train_critic(self, x_, y_, beta_, lr_train, k_prob):
        return self.sess.run([self.solver_L1_critic, self.LOSS_1, self.LOSS_2],
                             feed_dict={self.x: x_, self.y: y_, 
                                        self.beta:beta_,
                                        self.mb_size:np.shape(x_)[0], self.lr_rate1: lr_train, self.keep_prob: k_prob})
    
    def train_actor(self, x_, y_, alpha_, gamma_, lr_train, k_prob):
        return self.sess.run([self.solver_L1_actor, self.LOSS_1, self.LOSS_3],
                             feed_dict={self.x: x_, self.y: y_,
    ...
    def train_embedding(self, x_, y_, delta_, lr_train, k_prob):   
        return self.sess.run([self.solver_E, self.LOSS_1, self.LOSS_Ey], 
                             feed_dict={self.x:x_, self.y:y_,
                                        self.mb_size:np.shape(x_)[0], self.delta:delta_,
                                        self.lr_rate1:lr_train, self.keep_prob:k_prob})
